...
if echo "$CMD" | grep -qE -- "$PATTERN"; then
  echo "WRITE-SECRET GUARD: this command writes to a file and embeds a raw secret-shaped literal (private key / AWS access key / Slack bot token / GitLab PAT). Use a reference (env var, masked-cache path) instead of the literal value — never hardcode it into a script." >&2
  exit 2
fi
